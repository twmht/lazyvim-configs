...
local function substitute(command)
	...
	command = command:gsub("$fileBase", vim.fn.expand("%:r"))
	command = command:gsub("$filePath", vim.fn.expand("%:p"))
	command = command:gsub("$file", vim.fn.expand("%"))
	command = command:gsub("$dir", vim.fn.expand("%:p:h"))
	command = command:gsub("#", vim.fn.expand("#"))
	command = command:gsub("$altFile", vim.fn.expand("#"))
...
M.run_file = function(ht)
	...
	local command = fts[vim.bo.ft]
	vim.cmd(command and ("w | " .. (ht or "") .. "sp | term " .. command) or "echo 'No command for this filetype'")
end
